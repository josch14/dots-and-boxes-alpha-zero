...
class Trainer:
    ...
    model : AZNeuralNetwork
        the neural network that is updated in each iteration during model training
    best_model : AZNeuralNetwork
        the best neural network player (model may become best_model during evaluation)
    """
    ...
    def loop(self, n_iterations: int):
        ...
        for iteration in range(1, n_iterations + 1):
            ...
            print(f"Updated model won {round(win_percent * 100, 2)}% of the games vs. previous best model ({round(win_fraction * 100, 2)}% needed, draw = half win).")

            if win_percent >= win_fraction:
                iteration_of_best_model = iteration
                self.best_model = copy.deepcopy(self.model)
                print(f"Continuing Self-Play for data generation using updated model!")
                if self.model_name:
                    self.model.save_checkpoint(model_name=self.model_name + f"_{iteration}")
            else:
                print(f"Continuing Self-Play for data generation using best model from iteration {iteration_of_best_model}.")
    ...
    def perform_self_plays(game_size: int, model: AZNeuralNetwork, mcts_parameters: dict, n_workers: int, device: torch.device):
        ...
        Returns
        -------
        train_examples_per_game : [[[np.ndarray, [float], float]]]
    ...
    def perform_self_play(game_size: int, model: AZNeuralNetwork, mcts_parameters: dict):
        ...
        Returns
        -------
        train_examples : [[np.ndarray, [float], float]]
    ...
    def perform_model_training(model: AZNeuralNetwork, train_examples_per_game_augmented: list, data_parameters: dict, optimizer_parameters: dict, device: torch.device):
        ...
        ----------
        model : AZNeuralNetwork
            the neural network that is updated
        train_examples_per_game_augmented : [[[np.ndarray, [float], float]]]
            list (per game) of list of training examples (s, p, v) (from the current player's POV)
